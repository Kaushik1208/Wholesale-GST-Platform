"""
Sales Invoices page – view, filter, select and export sales invoices.
"""
...
def render():
    ...
    <div class='page-header'>
        ...
        <div class='page-subtitle'>All invoices issued to your customers</div>
    </div>
    """, unsafe_allow_html=True)

    rows = fetch_all("sales")
    ...
    if not rows:
        ...
        <div class='info-box' style='text-align:center;padding:60px 20px;background:var(--paper-2);border:1px solid var(--rule);border-radius:4px'>
            ...
            <div style='font-size:16px;font-weight:600;color:#6b6258'>No sales invoices yet</div>
            <div style='font-size:13px;color:#9e9590;margin-top:8px'>
                Go to <strong>Upload & Analyse</strong> and select type <strong>Sales</strong>
            </div>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    with st.expander("🔍 Filter Invoices", expanded=False):
        ...
        customer_filter = fc1.text_input("Customer Name contains", "")
    ...
    filtered = rows
    if customer_filter:
        filtered = [r for r in filtered if customer_filter.lower() in (r["vendor_name"] or "").lower()]
    ...
    if filtered:
        ...
        df_show.rename(columns={
            "id": "ID", "vendor_name": "Customer", "invoice_no": "Invoice No.",
    ...
    with c1:
        ...
        st.download_button(
            label,
            data=csv_data,
            file_name="sales_invoices_export.csv",
            mime="text/csv",
    ...
    with c2:
        st.download_button(
            "⬇️ Export All Sales Invoices",
            data=export_all("sales"),
            file_name="all_sales_invoices.csv",
            mime="text/csv",
            use_container_width=True,
        )
